...
//		return product;
		...
		try {
			result = (List<Member>) query.getResultList();
	...
	public Member findByCriteria(String userName) {
		...
		querySql.append(" SELECT * FROM Member ");
		if(StringUtils.isNotBlank(userName)) {
			querySql.append(" WHERE USER_NAME = '" + userName + "' ");
		}
		
		Query query = entityManager.createNativeQuery(querySql.toString(), Member.class);
		try {
			result = (List<Member>) query.getResultList();
		} catch(NoResultException ex) {
		}
